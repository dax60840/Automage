...
Shader "Unlit/s_CelShadingCutout" {
    Properties {
        ...
        _CycleColorImpact ("CycleColorImpact", Range(0, 1)) = 0
...
////// Emissive:
                ...
                float3 emissive = (lerp(float3(node_3248,node_3248,node_3248),_CycleColor.rgb,_CycleColorImpact)*((_LightColor0.rgb*max(_ShadowMinValue,(floor(((_posterizeBias*node_2302)+((dot((dot(i.normalDir,lightDirection)+UNITY_LIGHTMODEL_AMBIENT.rgb),float3(0.3,0.59,0.11))*attenuation)*step(node_1119,_FresnelWidth))) * _PosterizeSteps) / (_PosterizeSteps - 1)+UNITY_LIGHTMODEL_AMBIENT.rgb))*_Diffuse_var.rgb)*_LightnessMultiply));
