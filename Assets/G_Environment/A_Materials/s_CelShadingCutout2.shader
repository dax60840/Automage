...
Shader "Unlit/s_CelShadingAdditive" {
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform float _AdditiveMultiplier;
...
////// Lighting:
                ...
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float node_8422 = abs(pow(1.0-max(0,dot(normalDirection, viewDirection)),2.0));
                fixed4 finalRGBA = fixed4(finalColor * (pow(_Diffuse_var.g,3.0)+node_8422),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
